...
# n1 is the number of intervals per day
...
plot(oridata$interval[1:n1],interval_steps,type="l"
...
for (i in 1:n1){    
    newdata <- oridata$steps[extract]
    nas <- is.na(newdata)
    n2 <- length(newdata[nas])
    for(j in 1:n2){
        newdata[nas][j] <- interval_steps[i]
    }
    extract <- extract+1
}
# the code above only change the missing value in newdata 
# but the oridata has not changed
for (i in 1:n1){    
    nas <- is.na(oridata$steps[extract])
    n2 <- length(oridata$steps[extract][nas])
    for(j in 1:n2){
         oridata$steps[extract][nas][j] <- interval_steps[i]
     }
     extract <- extract+1
 }
...
png("his2.png")
hist(total_day2,breaks=seq(0,22000,2000),
     freq=TRUE,col="red",
